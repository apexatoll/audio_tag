module ID3
  ...
  # interface.
  #
  # Bang methods indicate that the read operation is not idempotent, as calling
  # them will result in the stream position being advanced. Non-bang methods
  # are idempotent, as they reset the stream position after reading meaning the
  # net effect on the stream position is 0.
  class StreamReader
    attr_reader :stream
    ...
    # The given byte is not included in the output, but the stream is advanced
    # to the position after it unless the consume_delimiter flag is false.
    #
    # This is useful for extracting string fragments until a delimiter without
    # including it in the extracted fragment nor the start of the next.
